...
The master saves timestamps when sending and receiving a package from which the round trip delay is calculated for every device in the system. 
...
There are two different types of packages which are transmitted over the network. The package which is broadcasted by the master, the \textit{update}, and the \textit{answer} which
