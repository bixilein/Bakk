...
The seconds of $t_1$ and the micro seconds of the timespamp $t_1$: \\
...
After the application has started in the first step a master has to be defined. 
Therefore a master election had to be designed as part of the program. There are two ways how a master can be found. The first one is to set a device as the 
master manually. This was implemented as a configuration option which can be set by the user in the configuration file. 
A master also can be found with a dynamic election. The dynamic election is put to use if no master was specified.

The user specifies a master: \\*
The user has the possibility to specify a master in the configuration file. To specify the master the option has to be set in the configuration file. 
If the configuration option is set there will be no dynamic election of the master. If two master would be specified a conflict would arise.
To prevent errors which would arise with two specified master only one master is allowed in a system. To make sure only one master exists at any time
within the same system one master is discarded if there are more than one. To decide which master has to be terminated the IPs of the devices are compared.
In the system basically all nodes are equal but each node in a local network has an individual IP address.
...
on the output for the user. The error message informs the user that only one master can be specified within the same group.

dynamic election of the master: \\*
If no master was specified by the user the election of the master is dynamic. The master election is implemented that a master can be found if 
several devices are turned on at the exact same time and also that if devices are added later to the system that they accept the current master.
Or that also a specified master can be added later to the system. It is also possible to change the configuration file during the runtime. 
Therefor a option to reconfigure had to be implemented in the program. The algorithm also accepts a specified master which was reconfigured during runtime.
