...
provided by so called sockets. The application creates the socket, where the socket type defines the communication typ. 
...
everyone transmission. The network broadcast address can be calculated by setting all the bits of the Host ID part to one.
...
\begin{itemize}
    \item periodically prepare and broadcast updates
...
two or more threads at the same time. This is a basic problem in concurrent computing, to protect resources from parallel access, mutexes are used.
A mutex is a mutual exclusion lock which can only be hold by one thread at a time.
%www.thinkingparallel.com/2006/09/09/mutual-exclusion-with-locks-an-introduction
One of the challenges in programming is to find all section in a code where shared data is accessed. It is really hard to debug a missing mutex, 
...
A mutex variable can have to sates: locked/set and unlocked/reset. 
If a thread wants to access a shared memory the \lstinline{pthread_mutex_lock()} function
\begin{itemize}
    \item locks the mutex, if it is currently unlocked
    \item blocks the calling thread, if the mutex is already locked by an ohter tread, untill it is unlocked again
    \item might block forever, if the mutex which was tired to be locked is already held by the current thread
\end{itemize}

If the tread finished the critical section the \lstinline{pthread_mutex_unlock()} function
\begin{itemize}
    \item unlocks the mutex, if the mutex was owned by the calling thread
    \item returns an error, if the mutex is already unlocked
    \item unlocks the mutex or returns an error, if the mutex was locked by an other tread, the behavior in this case depends on the type of the mutex.
\end{itemize} 
...
A mutex is created by calling \lstinline{pthread mutex init()} if \lstinline{NULL} is passed as argument to the function, the mutex will have the default 
attributes. The ID of the created mutex is returend to the calling thread through the mutex parameter. When a mutex is not needed anymore it should be
destroyed with \lstinline{pthread_mutex_destroy()}.


% something about muetx, why it is needed and how it works, just in general, but I really don't feel like doing that now
...
This threads prepares the updates and broadcasts them over the network. 
All information needed in the update (look up point xx master-update) has to be collected and copied in the message before the components get 
...
This threads waits for answers from slave devices, if an answer was received a time stamp is saved. In the next step the group IDs are compared and it is
...
\begin{itemize}
    \item receive updates and response to them
    \item communicate with the player
    \item initialize a event if necessary
\end{itemize}

All these tasks a performed in the function itself. In the slave mode the function \lstinline{recvfrom()} waits for an update from a master. 
If an update was received in the first step it is checked if the event ID has changed and if an event has to be initialized. If an event should be 
...
the answer and the answer is sent to the master the update was received from. Also the communication with the controller is done directly in the 
slave-mode function.
...
\subsection{Communication with pipe}
...
sync content should be loaded. The request for loading the content is transmitted over the network with the update and on each device the request is written to
...
(\lstinline{sync_rdy}) which adds the information to the answer for the master. If all devices have successfully loaded the next content, the event is 
